...
class handler extends msDB {
	...
	function getModule($id){
		$tmp = "";
		$sql = "select module from menu where id =?";
		$args = array (
				$id
		);
		$rs = $this->execSQL ( $sql, $args );
		if ($rs)
			$tmp = $rs->fields ['module'];
		return $tmp;
	}
	function getParent($id){
		$tmp = "";
		$sql = "select parent_id from menu where id =?";
		$args = array (
				$id
		);
		$rs = $this->execSQL ( $sql, $args );
		if ($rs)
			$tmp = $rs->fields ['parent_id'];
		return $tmp;
	}
	...
	function loadModel($file, $module = FALSE) {
		if (! $module) {
			$file_path = "app/model/{$file}.php";
		} else {
			$file_path = "app/{$module}/model/{$file}.php";
		}
