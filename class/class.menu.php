...
class Menu extends msDB {
	...
	function getChild($parent_id) {
		...
		if ($this->group == - 1) {
			$str_sql = "SELECT menu.id AS id, menu.title AS title, menu.iconcls as iconcls, menu.parent_id AS parent_id,
	...
	function getMenuJson($parent_id) {
		...
		while ( $row = $rs->FetchNextObject () ) {
			...
			$temp = ($temp == "") ? $temp : $temp . ",";
			$temp .= $this->tagJson ( $row->TITLE, $this->getMenuJson ( $row->ID ), $row->ID, $row->ICONCLS, $check );
		}
		return $temp;
	}
