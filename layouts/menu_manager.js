			...
			tbar : [
					...
					'-',
					{
						...
						handler : function() {
							...
							nodeS = treeMenu.getSelectionModel()
									.getSelectedNode();
							if (nodeS) {
								if (nodeS.getDepth() > 1) {
									var data = [];
			...
			bbar : [
					{
						...
						handler : function() {
							node = treeMenu.getSelectionModel()
									.getSelectedNode();
							if (node) {
								loadDetail(node);
							}
					...
					'-',
					{
						 ...
						 * ,'-',{ text:'Move Down', iconCls:'arrow-down',
						 * handler: function() { function clearS(node){
						 * treeMenu.getSelectionModel().clearSelections();
						 * //node.select(); } node =
						 * treeMenu.getSelectionModel().getSelectedNode(); if
						 * (!node) return false; if (node.getDepth() >1) if
						 * (!node.isLast()){ prevNode = node; x =
						 * node.nextSibling; parentNode = node.parentNode;
	...
	bbar : [
			{
				...
				handler : function() {
					detailMenu = Ext.getCmp('detailMenu');
					a = (detailMenu.getForm().getValues().menu_id == "") ? false
							: true;
					b = (detailMenu.getForm().getValues().parent_id == "") ? false
							: true;
					id_valid = a || b;
	...
	buttons : [
			{
				...
				handler : function() {
					...
					if (dataku.length) {
						if (handler_select == 'js')
							Ext.getCmp('detailMenu').findByType('trigger')[0].setValue(dataku[0].data.name);
						if (handler_select == 'php')
							Ext.getCmp('detailMenu').findByType('trigger')[1].setValue(dataku[0].data.name);
						if (handler_select == 'pdf')
							Ext.getCmp('detailMenu').findByType('trigger')[2].setValue(dataku[0].data.name);
						winHanlder.hide();
			...
			columns : [
					...
					},
					{
						...
						renderer : function(val, cell, rec) {
							return String
									.format(
											'<img class="x-panel-inline-icon " src="images/icon/{0}" /> {1}',
											rec.data.iconcls, val);
						}
					}, {
						dataIndex : 'sort_id',
						hidden : true,
										...
										.request({
											...
											success : function(response) {
												...
												if (res.success) {
													...
													Ext.example
															.msg('Save',
																	'Reording Menu successfully saved');
