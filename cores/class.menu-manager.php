...
class menuManager extends msDB {
	...
	function save(
				...
				$conf = Array(); 
				if ($menu_id) {
					$strSql = "update menu set title=?,module=?,iconCls=?,handler=?,report=?,ajax=?,published=? where id=?";
					$args = Array(
									$menu_title,
									$module,
									$iconCls,
					...
					$strSql = "insert into menu (parent_id,title,module,iconcls,handler,ajax,report,published)";
					$strSql .= " values(?,?,?,?,?,?,?,?);";
					$args = Array(
									$parent_id,
									$menu_title,
	 ...
	 * @return json string
	 */
	function handlerList($module='') {
		if ($module=='')
			$file = "app/view_js/";
		else 
			$file = "app/$module/view_js";
		$result = $this->fileList($file,"js");
		return $result; 
	}
	 ...
	 * @return json string
	 */
	function ajaxList($module='') {
		if ($module=='')
			$file = "app/controller/";
		else
			$file = "app/$module/controller/";
		$result = $this->fileList($file,"php");
		return $result; 
	}

	function reportList($module='') {
		if ($module=='')
			$file = "app/report/";
		else
			$file = "app/$module/report/";
		$result = $this->fileList($file,"php");
		return $result;
	}
